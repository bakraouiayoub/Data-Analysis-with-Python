...
def calculate_demographic_data(print_data=True):
    ...
    higher_education_rich = round(df.loc[(higher_education) & (df['salary']=='>50K')].shape[0]/higher_education.sum()*100,1)
    lower_education_rich = round(df.loc[(lower_education) & (df['salary']=='>50K')].shape[0]/lower_education.sum()*100,1)
    ...
    highest_earning_country = (rich_by_country/country_count).sort_values(ascending=False).index[0]
    highest_earning_country_percentage = round((rich_by_country/country_count).sort_values(ascending=False).iloc[0]*100,1) 
